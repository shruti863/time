...
# Define print functions for colored output
def print_color(text, color):
    print(f"{color}{text}{reset_text}")


# Define print functions with specific colors
print_brown = lambda text: print_color(text, brown_text)
print_yellow = lambda text: print_color(text, yellow_text)
print_red = lambda text: print_color(text, red_text)
print_green = lambda text: print_color(text, green_text)
print_cyan = lambda text: print_color(text, cyan_text)
print_blue = lambda text: print_color(text, blue_text)
print_magenta = lambda text: print_color(text, magenta_text)
print_pink = lambda text: print_color(text, pink_text)
print_purple = lambda text: print_color(text, purple_text)
print_golden = lambda text: print_color(text, golden_text)
print_teal = lambda text: print_color(text, teal_text)
print_orange = lambda text: print_color(text, orange_text)
print_midnight_blue = lambda text: print_color(text, midnight_blue_text)
print_khaki = lambda text: print_color(text, khaki_text)
print_coral = lambda text: print_color(text, coral_text)


# Function to get access tokens
def get_access_tokens():
    choice = get_colored_input(
...
# Function to get access tokens and thread IDs
...
# Function to send messages in round-robin fashion
        ...
        for j in range(num_tokens):
            ...
            for k in range(num_threads):
                ...
                if response and response.status_code == 200:
                    ...
                    print_red(
                        f"{red_text}Failed To Send Message Satish Sir Please Help {user_name} To Group Id {thread_id}: {message}{reset_text}")
